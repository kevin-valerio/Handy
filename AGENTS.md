...
- **Nix/NixOS**: upstream sets `HANDY_DISABLE_UPDATER=1` for immutable `/nix/store` installs. This fork removes the self-updater on all platforms so upstream releases cannot overwrite its cloud features.
